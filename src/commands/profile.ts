...
import { lockFile, profilePath, profilesDir } from '../session/paths.js';
...
export async function status(name = 'default'): Promise<void> {
  const profiles = await collectProfiles();
  const summary = profiles.find((p) => p.name === name) ?? (await summarizeProfile(name));
  if (!summary.exists && name !== 'default') {
    throw new CliError(2, 'NOT_FOUND', `Profile not found: ${name}`);
  }
...
async function summarizeProfile(name: string): Promise<ProfileSummary> {
  ...
  const locked = await pathExists(lockFile() + '.lock');
  const recent = (await readRecentEventSummaries(50)).reverse().find((s) => s.profile === name || (!s.profile && name === 'default'));
  return {
    name,
    path: dir,
    exists,
