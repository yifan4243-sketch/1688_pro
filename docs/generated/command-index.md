...
| `search` | `src/commands/search.ts` | `<keyword>` | `--max <n>`<br>`--sort <sort>`<br>`--price-min <n>`<br>`--price-max <n>`<br>`--province <name>`<br>`--city <name>`<br>`--verified <kind>`<br>`--min-turnover <n>`<br>`--exclude-ads`<br>`--deeppro`<br>`--deeppro-delay-min <seconds>`<br>`--deeppro-delay-max <seconds>`<br>`--profile <name>`<br>`--headed` | Search 1688 by keyword |
...
| `offer` | `src/commands/offer.ts` | `<offerIds...>` | `--profile <name>`<br>`--headed`<br>`--pro` | Show details of one or more 1688 offers |
